...
真实世界的 Agent 往往不止调用一个工具。视频里举了一个更复杂的例子："我马上要出门，该带些什么？如果下雨，帮我查一下附近有没有卖雨伞的店。"

这句话背后藏着三条工具调用，而且前一条的结果要喂给下一条：

| 步骤 | 工具 | 输入 | 输出 |
| --- | --- | --- | --- |
| 1 | 定位工具 | 用户当前位置 | 经纬度 |
| 2 | 天气工具 | 经纬度 | 是否下雨、气温、空气质量 |
| 3 | 店铺搜索工具 | 经纬度 + "雨伞" | 附近卖雨伞的店 |

大模型不会一次性全调，而是一步步来：先调定位拿到坐标，再决定要不要调天气，最后才决定要不要搜店铺。Function Calling 负责把每一步写成标准工单，Tool 负责真正去查，Skill 则把"出门清单"这套判断逻辑提前打包好——比如"下雨要带伞、空气差要戴口罩、风大要穿外套"。

这个例子把四层全部串起来了：决策层决定先调哪个，协议层输出每次调用的 JSON，执行层去碰定位/天气/地图接口，组织层的 Skill 提供完整的问题拆解与判断规则。
